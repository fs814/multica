...
func TestBuiltinSkillsConformToTemplate(t *testing.T) {
	...
	for _, skill := range skills {
		t.Run(skill.Name, func(t *testing.T) {
			...
			got := strings.TrimSpace(fm["allowed-tools"])
			if !strings.Contains(got, "Bash(multica *)") && !(skill.Name == "multica-workflows" && got == "Bash(multica workflow *)") {
...
func TestBuiltinSkillsFrontmatterIsStrictYAML(t *testing.T) {
	...
	for _, skill := range skills {
		t.Run(skill.Name, func(t *testing.T) {
			// embed preserves checkout line endings. Normalize CRLF so this
			// content contract is identical on Windows and Unix worktrees.
