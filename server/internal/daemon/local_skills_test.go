...
func writeTestClaudePlugin(t *testing.T, home, id, name string, enabled bool) string {
	...
	installed, err := json.Marshal(map[string]any{"version": 2, "plugins": map[string]any{id: []map[string]string{{"scope": "user", "installPath": installPath}}}})
	if err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(home, ".claude", "plugins", "installed_plugins.json"), installed, 0o644); err != nil {
		t.Fatal(err)
	}
