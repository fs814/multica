...
func (d *Daemon) handleModelList(ctx context.Context, rt Runtime, requestID string) {
	...
	// Built as a map rather than passed inline because the knot families append
	// one more key below; upstream's unavailable_models stays unconditional.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	// buildExecutionPrompt wraps BuildPrompt so the knot-http delivery path can
	// substitute a self-contained brief; it forwards promptOptions to BuildPrompt
	// for every other provider.
	prompt, err := d.buildExecutionPrompt(ctx, task, provider, promptOptions...)
	if err != nil {
	...
	if shouldRetryWithFreshSession(result, task.PriorSessionID, tools, provider) {
		...
		freshPrompt, err := d.buildExecutionPrompt(ctx, task, provider, promptOptions...)
		if err != nil {
