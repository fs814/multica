...
func TestIssuePoolWorkflowAcceptanceAndBoundedReworkRemainCanonical(t *testing.T) {
	definition := func(agentID string) *workflow.Definition {
		return &workflow.Definition{
			...
			EntryNode:     "implement",
			Nodes: []workflow.Node{
				{Key: "implement", Type: workflow.NodeTypeAgent, Next: []string{"acceptance"},
					Routing: &workflow.Routing{Strategy: workflow.RoutingExplicit, AgentID: agentID}, SubmissionSchema: "code_change"},
				{Key: "acceptance", Type: workflow.NodeTypeAcceptance, Next: []string{"end"},
					AcceptanceCriteria: []string{"result verified"}, ReworkTargets: []string{"implement"}},
				{Key: "end", Type: workflow.NodeTypeEnd},
			},
	...
	engine := &workflow.Engine{
		Queries: testHandler.Queries, TxStarter: testPool, Router: service.NewWorkflowRouter(testHandler.Queries),
		Schemas: workflow.DefaultSchemaRegistry,
	}
	testHandler.WorkflowEngine = engine
	t.Cleanup(func() { testHandler.WorkflowEngine = previous })

	start := func(key string) (string, IssuePoolCycleResponse) {
		...
		dbfx.QueryRow(t, `SELECT task_id::text FROM issue_pool_item WHERE id=$1`, cycle.Items[0].ID).Scan(&legacyTaskID)
		if legacyTaskID != nil {
			t.Fatalf("issue-pool item wrote legacy direct task %q", *legacyTaskID)
		}
		return autopilotID, cycle
	}
	latestStep := func(runID, node string) db.WorkflowStepInstance {
		steps, err := testHandler.Queries.ListWorkflowStepInstances(context.Background(), db.ListWorkflowStepInstancesParams{
			RunID: parseUUID(runID), WorkspaceID: parseUUID(testWorkspaceID),
		})
		if err != nil {
			t.Fatal(err)
		}
	...
	submit := func(runID, summary string) {
		step := latestStep(runID, "implement")
		if !step.TaskID.Valid {
			t.Fatal("Workflow Agent step has no task")
		}
		...
		var status, blockedReason, failureReason string
		dbfx.QueryRow(t, `SELECT status,COALESCE(blocked_reason,''),COALESCE(failure_reason,'') FROM workflow_run WHERE id=$1`, runID).Scan(&status, &blockedReason, &failureReason)
		if status != "waiting_acceptance" {
			t.Fatalf("submitted Workflow run = %s blocked=%q failed=%q", status, blockedReason, failureReason)
		}
	}
	decide := func(runID string, accept bool, reason string) {
		acceptanceStep := latestStep(runID, "acceptance")
		pending, err := testHandler.Queries.GetPendingWorkflowAcceptanceForStep(context.Background(), db.GetPendingWorkflowAcceptanceForStepParams{
			StepID: acceptanceStep.ID, WorkspaceID: parseUUID(testWorkspaceID),
		})
		if err != nil {
			t.Fatalf("load pending acceptance: %v", err)
		}
		input := workflow.DecideAcceptanceInput{WorkspaceID: parseUUID(testWorkspaceID), AcceptanceID: pending.ID,
			Accept: accept, ReviewerUserID: parseUUID(testUserID)}
		if !accept {
	...
	t.Run("accept completes only after human decision", func(t *testing.T) {
		autopilotID, cycle := start("accept")
		runID := *cycle.Items[0].WorkflowRunID
		submit(runID, "accepted result")
		ap, _ := testHandler.Queries.GetAutopilot(context.Background(), parseUUID(autopilotID))
		...
		if itemStatus != "waiting_acceptance" || cycleStatus != "waiting_acceptance" {
			var runStatus, blockedReason string
			dbfx.QueryRow(t, `SELECT status,COALESCE(blocked_reason,'') FROM workflow_run WHERE id=$1`, runID).Scan(&runStatus, &blockedReason)
			t.Fatalf("before acceptance item/cycle/run/reason = %s/%s/%s/%s", itemStatus, cycleStatus, runStatus, blockedReason)
		}
		decide(runID, true, "")
		testHandler.reconcileIssuePoolCycle(context.Background(), ap, parseUUID(cycle.ID))
		dbfx.QueryRow(t, `SELECT status FROM issue_pool_item WHERE id=$1`, cycle.Items[0].ID).Scan(&itemStatus)
	...
	t.Run("rejection consumes bounded Workflow rework and blocks in flight", func(t *testing.T) {
		autopilotID, cycle := start("bounded-rework")
		runID := *cycle.Items[0].WorkflowRunID
		submit(runID, "attempt one")
		decide(runID, false, "retry once")
		submit(runID, "attempt two")
		decide(runID, false, "budget exhausted")
		ap, _ := testHandler.Queries.GetAutopilot(context.Background(), parseUUID(autopilotID))
		testHandler.reconcileIssuePoolCycle(context.Background(), ap, parseUUID(cycle.ID))
		var itemStatus, cycleStatus string
		dbfx.QueryRow(t, `SELECT status FROM issue_pool_item WHERE id=$1`, cycle.Items[0].ID).Scan(&itemStatus)
		...
		if got := dbfx.Count(t, `SELECT count(*) FROM workflow_run WHERE idempotency_key=$1`, "issue-pool:"+cycle.Items[0].ID); got != 1 {
			t.Fatalf("bounded rework created %d WorkflowRuns", got)
		}
	})
}
