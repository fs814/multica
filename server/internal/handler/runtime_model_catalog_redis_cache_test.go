...
func TestRedisModelCatalogCache_RoundTrip(t *testing.T) {
	...
	if err := cache.Put(ctx, "runtime-1", sampleCatalog(), nil, true, nil); err != nil {
		t.Fatalf("put: %v", err)
	}
...
func TestRedisModelCatalogCache_SkipsUncacheableResults(t *testing.T) {
	...
	if err := cache.Put(ctx, "runtime-empty", nil, nil, true, nil); err != nil {
	...
	if err := cache.Put(ctx, "runtime-unsupported", sampleCatalog(), nil, false, nil); err != nil {
