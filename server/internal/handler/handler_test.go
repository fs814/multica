...
func TestMain(m *testing.M) {
	...
	// This fork encrypts autopilot trigger signing secrets at rest, so every
	// signing-secret endpoint fails closed with 503 when no box is configured
	// (see autopilotTriggerSigningSecret). Upstream's tests predate that gate
	// and assume the endpoints are reachable, so seed a box for the whole
	// suite. Tests that specifically exercise the unconfigured path still
	// clear it themselves with t.Cleanup.
	if box, boxErr := secretbox.New(make([]byte, secretbox.KeySize)); boxErr == nil {
		testHandler.VCSSecretBox = box
	} else {
		fmt.Printf("Failed to create test secretbox: %v\n", boxErr)
		pool.Close()
		os.Exit(1)
	}
	testPool = pool
