...
func (s *RedisModelListStore) Complete(ctx context.Context, id string, models []ModelEntry, unavailable []UnavailableModelEntry, supported bool, knotAgents []KnotAgentEntry) error {
	req, err := s.loadRequest(ctx, id)
	if err != nil {
		return err
	}
