...
func TestAcpThinkingDecision(t *testing.T) {
	...
	withCatalog := func(t *testing.T, models []ModelEntry) *Handler {
		...
		// Trailing nil is knotAgents: this fork's Knot agent list, which the
		// catalog cache carries alongside upstream's unavailable-model list.
		// Thinking-level tests seed neither.
		if err := cache.Put(ctx, uuidToString(runtimeID), models, nil, true, nil); err != nil {
