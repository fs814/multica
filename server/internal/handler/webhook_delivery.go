...
func (h *Handler) ReplayAutopilotDelivery(w http.ResponseWriter, r *http.Request) {
	...
	// Upstream moved run creation (and the trigger last_fired_at touch) into the
	// durable delivery worker, so the replay row is returned while still `queued`
	// and there is no run to report here. The fork's workflow-identity enrichment
	// is still applied: it no-ops when no autopilot run exists yet, and keeps this
	// response shape identical to the GET delivery endpoint once one does.
	resp := deliveryToResponse(replay, true)
	h.enrichWebhookDeliveryWorkflowIdentity(r, replay, &resp)
