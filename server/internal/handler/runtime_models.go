...
type ModelListStore interface {
	...
	// Complete records a successful discovery round. `unavailable` is the
	// advisory "named but will not run" list (never selectable — see the
	// invariant on UnavailableModelEntry). knotAgents is the knot/knot-http
	// agent list discovered alongside the models on the same round trip; nil
	// for every other provider.
	Complete(ctx context.Context, id string, models []ModelEntry, unavailable []UnavailableModelEntry, supported bool, knotAgents []KnotAgentEntry) error
...
func (s *InMemoryModelListStore) Complete(_ context.Context, id string, models []ModelEntry, unavailable []UnavailableModelEntry, supported bool, knotAgents []KnotAgentEntry) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (h *Handler) ReportModelListResult(w http.ResponseWriter, r *http.Request) {
	...
	if body.Status == "completed" {
		...
		if err := h.ModelListStore.Complete(r.Context(), requestID, body.Models, body.UnavailableModels, supported, body.KnotAgents); err != nil {
		...
		if h.ModelCatalogCache != nil {
			...
			case modelCatalogCacheStore:
				if err := h.ModelCatalogCache.Put(r.Context(), runtimeID, body.Models, body.UnavailableModels, supported, body.KnotAgents); err != nil {
