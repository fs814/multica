...
// matching human.
//
// The rule itself now lives in service.AgentInvokePermitted. It was moved there
// because the workflow Step router (service.WorkflowRouter) must apply the SAME
// gate and, running in package service, cannot call a *Handler method. Keeping
// this as a thin delegation - rather than letting the router re-derive the rule
// against the same tables - means there is one implementation to audit and one to
// fix. Every existing caller of canInvokeAgent is unaffected.
...
//
// The verdict itself is delegated to service.AgentInvokePermitted: the workflow
// Step router (service.WorkflowRouter) must apply the SAME gate and, running in
// package service, cannot call a *Handler method. Keeping this body a thin
// delegation - rather than re-deriving the rule against the same tables here -
// means there is one implementation to audit and one to fix.
func (h *Handler) invokeAgentDecision(ctx context.Context, agent db.Agent, actorType, actorID, originatorUserID, workspaceID string) bool {
	return service.AgentInvokePermitted(ctx, h.Queries, agent, service.InvokeActor{
		ActorType:        actorType,
		ActorID:          actorID,
