...
func TestStableIDSuffixMatchesDaemon(t *testing.T) {
	...
	// PredictRootDir joins with filepath.Join, so the expectation must too:
	// on Windows a "/"-concatenated string would never match.
	expected := filepath.Join(
		workspacesRoot,
		"asset-feed-"+taskDirSegment(workspaceID),
		"mul-6063-"+taskDirSegment(taskID),
	)
