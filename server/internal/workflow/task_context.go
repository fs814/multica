...
func SubmissionContractInstructions(stepInstanceID string) string {
	...
	b.WriteString("If you need clarification or cannot proceed, you must still submit: use verdict blocked and put the specific question or prerequisite in rationale. A standalone question is not a submission.\n\n")
	b.WriteString("The following is a valid blocked example. Replace it with your actual verdict and evidence; use pass only for verified work. Keep the markers on their own lines:\n\n")
	b.WriteString(submissionOpen)
	b.WriteString("\n")
	b.WriteString("{\n")
	b.WriteString(fmt.Sprintf("  %q: %d,\n", "schema_version", SchemaVersion))
	b.WriteString(fmt.Sprintf("  %q: %q,\n", "step_instance_id", stepInstanceID))
	b.WriteString(fmt.Sprintf("  %q: \"blocked\",\n", "verdict"))
