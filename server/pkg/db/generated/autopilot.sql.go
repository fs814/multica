...
const backfillAutopilotTriggerSigningSecret = `-- name: BackfillAutopilotTriggerSigningSecret :execrows
UPDATE autopilot_trigger
...
type BackfillAutopilotTriggerSigningSecretParams struct {
	SigningSecretEncrypted []byte      `json:"signing_secret_encrypted"`
	ID                     pgtype.UUID `json:"id"`
	LegacySigningSecret    pgtype.Text `json:"legacy_signing_secret"`
}
...
func (q *Queries) BackfillAutopilotTriggerSigningSecret(ctx context.Context, arg BackfillAutopilotTriggerSigningSecretParams) (int64, error) {
	result, err := q.db.Exec(ctx, backfillAutopilotTriggerSigningSecret, arg.SigningSecretEncrypted, arg.ID, arg.LegacySigningSecret)
	if err != nil {
		return 0, err
	}
...
) RETURNING id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, squad_id, planned_at, webhook_delivery_id, workflow_run_id, quota_reservation_id, reason_code
`
...
INSERT INTO autopilot_trigger (
    ...
    $11, $12,
    $13, $14
) RETURNING id, autopilot_id, kind, enabled, cron_expression, timezone, next_run_at, webhook_token, label, last_fired_at, created_at, updated_at, provider, signing_secret, event_filters, published_by_type, published_by_id, signing_secret_encrypted, created_by_type, created_by_id
`
...
SELECT id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, squad_id, planned_at, webhook_delivery_id, workflow_run_id, quota_reservation_id, reason_code FROM updated_runs
`
...
SELECT id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, squad_id, planned_at, webhook_delivery_id, workflow_run_id, quota_reservation_id, reason_code FROM autopilot_run
WHERE id = $1
`
...
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, workflow_step_instance_id, branch_name, durable_work_dir, channel_context_revision, comment_thread_id, cancelled_by_type, cancelled_by_id, cancelled_by_name, issue_snapshot, debug_never_dispatched_at FROM agent_task_queue
...
SELECT id, autopilot_id, kind, enabled, cron_expression, timezone, next_run_at, webhook_token, label, last_fired_at, created_at, updated_at, provider, signing_secret, event_filters, published_by_type, published_by_id, signing_secret_encrypted, created_by_type, created_by_id FROM autopilot_trigger
WHERE id = $1
`
...
SELECT t.id, t.autopilot_id, t.kind, t.enabled, t.cron_expression, t.timezone, t.next_run_at, t.webhook_token, t.label, t.last_fired_at, t.created_at, t.updated_at, t.provider, t.signing_secret, t.event_filters, t.published_by_type, t.published_by_id, t.signing_secret_encrypted, t.created_by_type, t.created_by_id, a.workspace_id AS autopilot_workspace_id
...
const listLegacyAutopilotTriggerSigningSecrets = `-- name: ListLegacyAutopilotTriggerSigningSecrets :many
...
type ListLegacyAutopilotTriggerSigningSecretsRow struct {
	ID            pgtype.UUID `json:"id"`
	SigningSecret pgtype.Text `json:"signing_secret"`
}
...
func (q *Queries) ListLegacyAutopilotTriggerSigningSecrets(ctx context.Context) ([]ListLegacyAutopilotTriggerSigningSecretsRow, error) {
	rows, err := q.db.Query(ctx, listLegacyAutopilotTriggerSigningSecrets)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListLegacyAutopilotTriggerSigningSecretsRow{}
	for rows.Next() {
		var i ListLegacyAutopilotTriggerSigningSecretsRow
...
func (q *Queries) SetAutopilotTriggerSigningSecret(ctx context.Context, arg SetAutopilotTriggerSigningSecretParams) (AutopilotTrigger, error) {
	row := q.db.QueryRow(ctx, setAutopilotTriggerSigningSecret, arg.ID, arg.SigningSecretEncrypted)
	var i AutopilotTrigger
	err := row.Scan(
		&i.ID,
		&i.AutopilotID,
...
UPDATE autopilot SET
    ...
    workflow_template_version_id = $11,
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, title, description, assignee_id, status, execution_mode, issue_title_template, created_by_type, created_by_id, last_run_at, created_at, updated_at, assignee_type, project_id, workflow_template_id, workflow_template_version_id, pause_reason
`
...
WITH updated_run AS (
    ...
    SET status = $1::text,
        ...
        END
    WHERE ar.id = $5
    RETURNING ar.id, ar.autopilot_id, ar.trigger_id, ar.source, ar.status, ar.issue_id, ar.task_id, ar.triggered_at, ar.completed_at, ar.failure_reason, ar.trigger_payload, ar.result, ar.created_at, ar.squad_id, ar.planned_at, ar.webhook_delivery_id, ar.workflow_run_id, ar.quota_reservation_id, ar.reason_code
), locked_reservation AS MATERIALIZED (
    SELECT qr.id, qr.workspace_id, qr.period_start, qr.period_end, qr.policy_revision, qr.subscription_version, qr.source, qr.idempotency_key, qr.state, qr.created_at, qr.finalized_at
    FROM autopilot_quota_reservation qr
    JOIN updated_run ar ON ar.quota_reservation_id = qr.id
...
SELECT id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, squad_id, planned_at, webhook_delivery_id, workflow_run_id, quota_reservation_id, reason_code FROM updated_run
`
...
const updateAutopilotRunWorkflowRunning = `-- name: UpdateAutopilotRunWorkflowRunning :one
UPDATE autopilot_run
...
type UpdateAutopilotRunWorkflowRunningParams struct {
	ID            pgtype.UUID `json:"id"`
	WorkflowRunID pgtype.UUID `json:"workflow_run_id"`
	IssueID       pgtype.UUID `json:"issue_id"`
}

func (q *Queries) UpdateAutopilotRunWorkflowRunning(ctx context.Context, arg UpdateAutopilotRunWorkflowRunningParams) (AutopilotRun, error) {
	row := q.db.QueryRow(ctx, updateAutopilotRunWorkflowRunning, arg.ID, arg.WorkflowRunID, arg.IssueID)
	var i AutopilotRun
	err := row.Scan(
		&i.ID,
		&i.AutopilotID,
