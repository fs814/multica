...
type Config struct {
	ExecutablePath string            // path to CLI binary (claude, codebuddy, codex, copilot, opencode, codearts, openclaw, hermes, pi, cursor, kimi, reasonix, dsh, kiro-cli, agy, qodercli, qoderclicn, traecli, grok, qwen, qwenpaw, mcode, dim, zeroclaw, knot-cli)
...
// Supported types: "claude", "codebuddy", "codex", "copilot", "opencode", "codearts", "deveco", "openclaw", "hermes", "pi", "cursor", "kimi", "reasonix", "dsh", "kiro", "antigravity", "qoder", "qoderclicn", "traecli", "grok", "qwen", "qwenpaw", "mcode", "knot", "knot-http".
//
...
// migration 254 to add reasonix, migration 282 to add knot, migration 283 to
// add knot-http, migration 313 to add dsh, migration 342 to
// add mcode, migration 370 to add dim, migration 403 to add zeroclaw, and
// migration 441 to add codearts — which dropped knot and knot-http, since it
// rewrites the whole IN list and predates neither; migration 500 restores the
// full union and is the one to widen next): a custom runtime profile may
