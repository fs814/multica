    ...
    try {
      ...
      if (isCreate) {
        const autopilot = await createAutopilot.mutateAsync({
          ...
          // Upstream MUL-6681 (#7553) made Project apply to every execution
          // mode, run_only included; issue_pool needs it for the same reason
          // (the pool scans a project's backlog), so no mode nulls it here.
      ...
      >
        ...
        >
          ...
          <aside className="w-full lg:w-[380px] shrink-0 overflow-visible lg:overflow-y-auto px-5 py-5 space-y-5 bg-muted/30">
            ...
            <OutputModeSection mode={executionMode} onChange={handleExecutionModeChange} assigneeType={assigneeType} />
            ...
            {/* Shown for EVERY output mode (MUL-6681). The project is not only
