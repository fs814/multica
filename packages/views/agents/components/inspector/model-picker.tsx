...
export function ModelPicker({
  ...
  const picker = (
    <PropertyPicker
      ...
      header={
        // Upstream's shared header (search + refresh). The discovery hint stays
        // a sibling rather than a header prop: it is the only part of this
        // surface that reports runtime state ("runtime offline, type a model
        // manually"), and the header is reused by the non-inspector dropdown
        // which renders that message in its own label row instead.
        <>
          <ModelSearchHeader
            value={search}
            onChange={setSearch}
          ...
          />
          {discoveryHint && (
            <p role="status" className="px-1.5 pb-1.5 text-caption text-muted-foreground">
              {discoveryHint}
            </p>
          )}
        </>
