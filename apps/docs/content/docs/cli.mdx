...
| `autopilot` | `list/get/create/update/delete` | Manage automations | `create`: `--title`, `--agent`, `--mode` (all required), `--description` / `--description-stdin` / `--description-file`, `--project`, `--subscriber` (repeatable); `update` accepts the same description inputs; `--allow-external-file` opts file input out of the workdir guard |
